...
6. 输出外层文件夹使用中文命名，内部图片保留 `front / back / left / right` 标准视图名。
7. 图像风格以 Tripo 建模可用为目标：低模大面、结构清楚、少毛刺、少碎裂，方便后续 Blender 精简。
8. 如果 Tripo 生成结果出现灰白素模、碎面、结构脏、材质不明显，不要第一时间判断为面数太低；优先生成一版“建模友好版四视图”作为补救输入。
